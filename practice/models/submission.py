...
class Submission(models.Model):
    ...
    profile = models.ForeignKey("backend.Profile", verbose_name=_("profile"), related_name='practice_submissions', on_delete=models.CASCADE, null=True)
    contest = models.ForeignKey("practice.Practice", verbose_name=_("contest"), on_delete=models.CASCADE, null=True)
    ...
    def judge(self):
        self.points = 0.0
        self.max_points = 0.0
        for problem in self.contest.problems.all():
    ...
    class Meta:
        permissions = (
            ('view_all_practice_submission', _('View all practice submission')),
            ('view_output_practice_submission', _('View user practice submission output'))
        )
        verbose_name = _('practice submission')
        verbose_name_plural = _('practice submissions')


class SubmissionProblem(models.Model):
    submission = models.ForeignKey("practice.Submission", verbose_name=_("submission"), 
                                   related_name='problems', on_delete=models.CASCADE)
    problem = models.ForeignKey("practice.PracticeProblem", verbose_name=_("contest problem"), on_delete=models.CASCADE, null=True)
    task = models.ForeignKey("education.Problem", verbose_name=_("problem"), on_delete=models.CASCADE, related_name='practice_problem_submissions', null=True)
